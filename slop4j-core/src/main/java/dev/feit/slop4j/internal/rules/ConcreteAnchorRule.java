...
public final class ConcreteAnchorRule implements SlopRule {
	...
	static final Pattern FILE_NAME = Pattern
			.compile("\\b[\\w.-]+\\.(java|ts|js|json|yaml|yml|xml|md|sql|sh|properties)\\b");
	static final Pattern METHOD_NAME = Pattern.compile("\\b[a-z][a-zA-Z0-9]*\\s*\\(");
	static final Pattern CLASS_NAME = Pattern.compile(
			"\\b[A-Z][A-Za-z0-9]+(?:Service|Controller|Repository|Factory|Builder|Config|Configuration|Exception|Client|Mapper)\\b");
	static final Pattern CLI_COMMAND = Pattern
			.compile("(?m)^\\s*(mvn|npm|npx|pnpm|yarn|git|docker|kubectl|helm|java|curl)\\b.*$");
	...
	public Result analyze(SlopContext context, int maxFindingEvidenceLength) {
		...
		if (context.tokenCount() >= 80 && evidenceScore < 0.2) {
			findings.add(new SlopFinding(SlopFindingType.LOW_EVIDENCE, Severity.WARNING,
					"Text contains few concrete anchors such as numbers, versions, commands or file names.", ""));
			findings.add(new SlopFinding(SlopFindingType.LOW_CONCRETENESS, Severity.WARNING,
					"Text provides limited concrete implementation detail.", ""));
		} else if (context.tokenCount() >= 20 && evidenceScore < 0.2) {
			findings.add(new SlopFinding(SlopFindingType.LOW_CONCRETENESS, Severity.INFO,
					"Text provides limited concrete implementation detail.", ""));
		}
	...
	private static double collect(String text, Pattern pattern, double weight, List<String> anchors) {
