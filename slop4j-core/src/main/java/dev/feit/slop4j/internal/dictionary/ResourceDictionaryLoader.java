...
public final class ResourceDictionaryLoader {
	...
	private LanguageDictionary read(Language language) {
		...
		try (InputStream input = ResourceDictionaryLoader.class.getResourceAsStream(resourcePath)) {
			...
			if (!(loaded instanceof Map<?, ?> map)) {
				throw new IllegalStateException("Dictionary resource must be a YAML mapping: " + resourcePath);
			}
			return new LanguageDictionary(stringList(map, "buzzwords", resourcePath),
					stringList(map, "abstractNouns", resourcePath), stringList(map, "vaguePhrases", resourcePath),
					stringList(map, "weaselWords", resourcePath), stringList(map, "activeVerbs", resourcePath),
					stringList(map, "concreteTerms", resourcePath), stringList(map, "claimMarkers", resourcePath));
		} catch (IOException exception) {
			throw new IllegalStateException("Unable to read dictionary resource: " + resourcePath, exception);
		}
	}
	...
	private static List<String> stringList(Map<?, ?> map, String key, String resourcePath) {
		...
		if (!(value instanceof List<?> list)) {
			throw new IllegalStateException("Dictionary field must be a YAML sequence: " + resourcePath + "#" + key);
		}
	...
	private static final class DictionarySetBuilder {
		...
		DictionarySet build() {
			return new DictionarySet(unmodifiableSet(buzzwords), unmodifiableSet(abstractNouns),
					List.copyOf(vaguePhrases), unmodifiableSet(weaselWords), unmodifiableSet(activeVerbs),
					unmodifiableSet(concreteTerms), unmodifiableSet(claimMarkers));
		}
		...
		private void addPhrases(Collection<String> values) {
			for (String value : values) {
				normalize(value).ifPresent(normalized -> {
