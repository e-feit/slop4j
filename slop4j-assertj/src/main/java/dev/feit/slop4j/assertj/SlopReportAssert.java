...
public final class SlopReportAssert extends AbstractAssert<SlopReportAssert, SlopReport> {
	...
	/**
	 * Requires the slop score to be lower than the exclusive threshold in the
	 * {@code 0.0..100.0} range.
	 */
	public SlopReportAssert hasSlopScoreBelow(double maximumExclusive) {
		assertNumeric("slop score", SlopReport::slopScore, maximumExclusive, 100.0, "below",
				actualValue -> actualValue < maximumExclusive);
		return this;
	}

	/**
	 * Requires the slop score to be at most the inclusive threshold in the
	 * {@code 0.0..100.0} range.
	 */
	public SlopReportAssert hasSlopScoreAtMost(double maximumInclusive) {
		assertNumeric("slop score", SlopReport::slopScore, maximumInclusive, 100.0, "at most",
				actualValue -> actualValue <= maximumInclusive);
		return this;
	}

	/**
	 * Requires the slop score to be higher than the exclusive threshold in the
	 * {@code 0.0..100.0} range.
	 */
	public SlopReportAssert hasSlopScoreAbove(double minimumExclusive) {
		assertNumeric("slop score", SlopReport::slopScore, minimumExclusive, 100.0, "above",
				actualValue -> actualValue > minimumExclusive);
		return this;
	}

	/**
	 * Requires the slop score to be at least the inclusive threshold in the
	 * {@code 0.0..100.0} range.
	 */
	public SlopReportAssert hasSlopScoreAtLeast(double minimumInclusive) {
		assertNumeric("slop score", SlopReport::slopScore, minimumInclusive, 100.0, "at least",
				actualValue -> actualValue >= minimumInclusive);
		return this;
	}

	/**
	 * Requires buzzword density to be lower than the exclusive threshold in the
	 * {@code 0.0..1.0} range.
	 */
	public SlopReportAssert hasBuzzwordDensityBelow(double maximumExclusive) {
		assertUnitScore("buzzword density", SlopReport::buzzwordDensity, maximumExclusive, "below",
				actualValue -> actualValue < maximumExclusive);
		return this;
	}

	/**
	 * Requires buzzword density to be higher than the exclusive threshold in the
	 * {@code 0.0..1.0} range.
	 */
	public SlopReportAssert hasBuzzwordDensityAbove(double minimumExclusive) {
		assertUnitScore("buzzword density", SlopReport::buzzwordDensity, minimumExclusive, "above",
				actualValue -> actualValue > minimumExclusive);
		return this;
	}

	/**
	 * Requires vague phrase density to be lower than the exclusive threshold in the
	 * {@code
	 * 0.0..1.0} range.
	 */
	public SlopReportAssert hasVaguePhraseDensityBelow(double maximumExclusive) {
		assertUnitScore("vague phrase density", SlopReport::vaguePhraseDensity, maximumExclusive, "below",
				actualValue -> actualValue < maximumExclusive);
		return this;
	}

	/**
	 * Requires concreteness score to be higher than the exclusive threshold in the
	 * {@code 0.0..1.0} range.
	 */
	public SlopReportAssert hasConcretenessScoreAbove(double minimumExclusive) {
		assertUnitScore("concreteness score", SlopReport::concretenessScore, minimumExclusive, "above",
				actualValue -> actualValue > minimumExclusive);
		return this;
	}

	/**
	 * Requires actionability score to be higher than the exclusive threshold in the
	 * {@code
	 * 0.0..1.0} range.
	 */
	public SlopReportAssert hasActionabilityScoreAbove(double minimumExclusive) {
		assertUnitScore("actionability score", SlopReport::actionabilityScore, minimumExclusive, "above",
				actualValue -> actualValue > minimumExclusive);
		return this;
	}

	/**
	 * Requires evidence score to be higher than the exclusive threshold in the
	 * {@code 0.0..1.0} range.
	 */
	public SlopReportAssert hasEvidenceScoreAbove(double minimumExclusive) {
		assertUnitScore("evidence score", SlopReport::evidenceScore, minimumExclusive, "above",
				actualValue -> actualValue > minimumExclusive);
		return this;
	}

	/**
	 * Requires repetition score to be lower than the exclusive threshold in the
	 * {@code 0.0..1.0} range.
	 */
	public SlopReportAssert hasRepetitionScoreBelow(double maximumExclusive) {
		assertUnitScore("repetition score", SlopReport::repetitionScore, maximumExclusive, "below",
				actualValue -> actualValue < maximumExclusive);
		return this;
	}

	/**
	 * Requires overconfidence score to be lower than the exclusive threshold in the
	 * {@code
	 * 0.0..1.0} range.
	 */
	public SlopReportAssert hasOverconfidenceScoreBelow(double maximumExclusive) {
		assertUnitScore("overconfidence score", SlopReport::overconfidenceScore, maximumExclusive, "below",
				actualValue -> actualValue < maximumExclusive);
		return this;
	}
	...
	public SlopReportAssert hasVerdict(SlopVerdict expected) {
		...
		if (report.verdict() != expected) {
			failWithMessage("Expected verdict to be %s, but was %s.%n%n%s", expected, report.verdict(),
					formatReport(report));
		}
		return this;
	}
	...
	public SlopReportAssert doesNotHaveVerdict(SlopVerdict unexpected) {
		...
		if (report.verdict() == unexpected) {
			failWithMessage("Expected verdict not to be %s, but was %s.%n%n%s", unexpected, report.verdict(),
					formatReport(report));
		}
		return this;
	}
	...
	public SlopReportAssert hasFindingOfType(SlopFindingType expectedType) {
		...
		if (report.findings().stream().noneMatch(finding -> finding.type() == expectedType)) {
			failWithMessage("Expected finding of type %s, but available findings were:%n%s", expectedType,
	...
	public SlopReportAssert hasNoFindingOfType(SlopFindingType unexpectedType) {
		...
		SlopReport report = requireReport();
		var matches = report.findings().stream().filter(finding -> finding.type() == unexpectedType).toList();
		if (!matches.isEmpty()) {
			failWithMessage("Expected no finding of type %s, but found:%n%s", unexpectedType, formatFindings(matches));
		}
		return this;
	}
	...
	public SlopReportAssert hasFindingWithSeverity(Severity expectedSeverity) {
		...
		SlopReport report = requireReport();
		if (report.findings().stream().noneMatch(finding -> finding.severity() == expectedSeverity)) {
			failWithMessage("Expected finding with severity %s, but available findings were:%n%s", expectedSeverity,
					formatFindings(report));
		}
		return this;
	}
	...
	/**
	 * Requires concreteness score above {@code 0.5} or evidence score above
	 * {@code 0.5}.
	 */
	...
	public SlopReportAssert lacksConcreteDetails() {
		...
		if (!(report.concretenessScore() < 0.25 && report.evidenceScore() < 0.25)) {
			failWithMessage("Expected concreteness score and evidence score to be below 0.25.%n%n%s",
					formatReport(report));
		}
		return this;
	}
	...
	public SlopReportAssert isActionable() {
		...
		if (report.actionabilityScore() <= 0.5) {
			failWithMessage("Expected actionability score to be above 0.5.%n%n%s", formatReport(report));
		}
		return this;
	}

	/**
	 * Requires actionability score below {@code 0.25} or a low-actionability
	 * finding.
	 */
	public SlopReportAssert isInsufficientlyActionable() {
		SlopReport report = requireReport();
		boolean hasLowActionabilityFinding = report.findings().stream()
		...
		if (!(report.actionabilityScore() < 0.25 || hasLowActionabilityFinding)) {
			failWithMessage("Expected actionability score below 0.25 or a LOW_ACTIONABILITY finding.%n%n%s",
					formatReport(report));
		}
		return this;
	}
	...
	public SlopReportAssert doesNotSoundLikeLinkedInPost() {
		...
		if (v == SlopVerdict.LINKEDIN_READY || v == SlopVerdict.PREMIUM_POLISHED_GARBAGE
				|| v == SlopVerdict.BOARD_APPROVED_SLOP || v == SlopVerdict.GARBAGE_IN_SLOP_OUT
				|| v == SlopVerdict.CERTIFIED_BRAINLESS_SLOP) {
			failWithMessage("Expected verdict not to be LINKEDIN_READY or higher slop severity.%n%n%s",
					formatReport(report));
		}
		return this;
	}
	...
	public SlopReportAssert soundsLikeLinkedInPost() {
		...
		if (!(v == SlopVerdict.LINKEDIN_READY || v == SlopVerdict.PREMIUM_POLISHED_GARBAGE
				|| v == SlopVerdict.BOARD_APPROVED_SLOP || v == SlopVerdict.GARBAGE_IN_SLOP_OUT
				|| v == SlopVerdict.CERTIFIED_BRAINLESS_SLOP)) {
			failWithMessage("Expected verdict to be LINKEDIN_READY or higher slop severity.%n%n%s",
					formatReport(report));
		}
		return this;
	}
	...
	public SlopReportAssert isBoardDeckReady() {
		SlopReport report = requireReport();
		SlopVerdict v = report.verdict();
		if (!(v == SlopVerdict.BOARD_APPROVED_SLOP || v == SlopVerdict.GARBAGE_IN_SLOP_OUT
				|| v == SlopVerdict.CERTIFIED_BRAINLESS_SLOP || report.slopScore() >= 85.0)) {
			failWithMessage("Expected verdict BOARD_APPROVED_SLOP or higher slop severity.%n%n%s",
					formatReport(report));
		}
		return this;
	}
	...
	/**
	 * Requires verdict {@code CERTIFIED_BRAINLESS_SLOP} or slop score at least
	 * {@code 97.0}.
	 */
	public SlopReportAssert isBrainlessSlop() {
		SlopReport report = requireReport();
		if (!(report.verdict() == SlopVerdict.CERTIFIED_BRAINLESS_SLOP || report.slopScore() >= 97.0)) {
			failWithMessage("Expected verdict CERTIFIED_BRAINLESS_SLOP or slop score at least 97.0.%n%n%s",
					formatReport(report));
		}
		return this;
	}
	...
	/**
	 * Requires evidence score below {@code 0.2} and concreteness score below
	 * {@code 0.3}.
	 */
	public SlopReportAssert containsNoImplementationDetails() {
		SlopReport report = requireReport();
		if (!(report.evidenceScore() < 0.2 && report.concretenessScore() < 0.3)) {
			failWithMessage("Expected evidence score below 0.2 and concreteness score below 0.3.%n%n%s",
					formatReport(report));
		}
		return this;
	}
	...
	/**
	 * Requires slop score at least {@code 75.0}, actionability score below
	 * {@code 0.3}, and evidence score below {@code 0.3}.
	 */
	public SlopReportAssert maximizesPlausibleDeniability() {
		SlopReport report = requireReport();
		if (!(report.slopScore() >= 75.0 && report.actionabilityScore() < 0.3 && report.evidenceScore() < 0.3)) {
			failWithMessage(
	...
	private void assertUnitScore(String label, ToDoubleFunction<SlopReport> extractor, double threshold,
			String relation, DoubleCondition condition) {
		assertNumeric(label, extractor, threshold, 1.0, relation, condition);
	}

	private void assertNumeric(String label, ToDoubleFunction<SlopReport> extractor, double threshold, double maximum,
			String relation, DoubleCondition condition) {
		...
		if (!condition.matches(actualValue)) {
			failWithMessage("Expected %s to be %s %s, but was %s.%n%n%s", label, relation, threshold, actualValue,
					formatReport(report));
		}
	}
	...
	private static void validateThreshold(String label, double threshold, double maximum) {
		if (!Double.isFinite(threshold) || threshold < 0.0 || threshold > maximum) {
			throw new IllegalArgumentException(label + " threshold must be finite and within 0.0.." + maximum + ".");
		}
	}
	...
	private static String formatReport(SlopReport report) {
		return """
				...
				Findings:
				%s""".formatted(report.verdict(), report.slopScore(), report.buzzwordDensity(),
				report.vaguePhraseDensity(), report.concretenessScore(), report.actionabilityScore(),
				report.evidenceScore(), report.repetitionScore(), report.overconfidenceScore(), formatFindings(report));
	}
	...
	private static String formatFindings(Iterable<SlopFinding> findings) {
		...
		for (SlopFinding finding : findings) {
			...
			builder.append("  - ").append(finding.type()).append(' ').append(finding.severity()).append(": ")
